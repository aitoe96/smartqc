...
#' Load necessary data for SmartQC
#'
#' @param org Organism (MOUSE, HUMAN, or RAT) given by user at start
#' @return A list containing necessary marker data for SCINA and necessary cell cycle genes data for vst regress out
#' @export
load_necessary_data <- function(org) {
  org <- toupper(org)
  if (!org %in% c("MOUSE", "HUMAN", "RAT")) {
    stop("Invalid organism. Must be MOUSE, HUMAN, or RAT.")
  }
  
  #get package path
  pkg_path <- system.file(package = "smartqc")
  #cat("Debug: Package path:", pkg_path, "\n")
  
  #findalldata
  all_data <- data(package = "smartqc")$results[, "Item"]
  #cat("Debug: Available data:", paste(all_data, collapse = ", "), "\n")
  
  #get only species-compliant data
  cm_data_name <- paste0("cm_", tolower(org))
  
  #cat("Debug: Looking for", cm_data_name, "and cellcyclegenes\n")
  
   if (!(cm_data_name %in% all_data)) {
    stop("Cell markers data not found for organism: ", org)
  }
  if (!("cellcyclegenes" %in% all_data)) {
    stop("Cell cycle genes data not found")
  }
  
    data(list = c(cm_data_name, "cellcyclegenes"), envir = environment())
  
  cm_data <- get(cm_data_name)
  cc_genes <- cellcyclegenes[[org]]
  
  #cat("Debug: Data loaded successfully\n")
  #cat("Debug: cm_data class:", class(cm_data), "\n")
  #cat("Debug: cc_genes class:", class(cc_genes), "\n")
  #cat("Debug: cc_genes dimensions:", paste(dim(cc_genes), collapse = "x"), "\n")
  
  return(list(cell_markers = cm_data, cell_cycle_genes = cc_genes))
}



##TO DO: option to rerun ensembl to get old data
##TO DO: check if we still use this below or what
       # org <- toupper(org)
  # if (!org %in% c("MOUSE", "HUMAN", "RAT")) {
  #   stop("Invalid organism. Must be MOUSE, HUMAN, or RAT.")
  # }
  ...
  # cm_data <- get(load(file.path(pkg_path, "data", paste0("cm_", tolower(org), ".rda"))))
  # cc_genes <- get(load(file.path(pkg_path, "data", paste0("CellCycleGenes_", org, ".RData"))))
  # 
  # return(list(cell_markers = cm_data, cell_cycle_genes = cc_genes))
...
#'
#' This function filters cells based on percentage of mitochondrial transcripts,
#' number of genes, and number of transcripts detected. 
#' A median absolute deviation approach, letting the distribution of the data define cutoffs, is chosen
#'
#' @param seur A Seurat object
#' @param mad.coeff Coefficient for median absolute deviation, default is 3
#' @param pass An identifier for the number of times this function is running, used for plotting
#' @param org The organism the data is from ("HUMAN", "MOUSE", or "RAT")
#' @param output_path Directory to save output plots, default is output_dir
#'
#' @return A filtered Seurat object
#' @export
#'
#' @examples
#' filtered_seurat <- filterCells(seurat_obj, mad.coeff = 3, pass = 1, org = "HUMAN")
filterCells <- function(seur,mad.coeff = 3, 
                        min.features.thr = NULL, 
                        max.features.thr = NULL,
                        max.mito.thr = NULL, 
                        min.mito.thr = NULL,
                        max.count.thr = NULL,
                        pass = -1,org = "HUMAN", output_path = output_dir){
  ...
  #filter cells based on percentage of mitochondrial transcripts
  cell.QC.stat <- seur@meta.data

  if (is.null(max.mito.thr)){
...
#' It processes the data, selects appropriate markers based on the organism and tissue,
#' and applies the SCINA algorithm for cell type annotation.
#'
#' @param seur A Seurat object containing the single-cell RNA-seq data.
#' @param org Character string specifying the organism. Must be one of "HUMAN", "MOUSE", or "RAT".
#' @param tissue Character string specifying the tissue type. Default is "ALL".
#' @param id.type Character string specifying the gene ID type. Must be either "Symbol" or "Ensembl".
#' @param output_path Character string specifying the path to save the UMAP plot.
#'   Default is "UMAP_SCINA.png" in the output_dir.
#'
#' @return A Seurat object with added SCINA annotations in the meta.data slot.
#' @export
#'
#' @examples
#' seur <- annotateCells(seur, org = "HUMAN", tissue = "Liver", id.type = "Symbol")
#'
...
annotateCells <- function(seur,org = c("HUMAN","MOUSE","RAT"), tissue = "ALL",id.type = c("Symbol","Ensembl"),  output_path = paste0(output_dir,"/UMAP_SCINA.png")){
...
runDensUMAP <- function(seur, pcs, output_dir = ".") {
  ...
  result <- tryCatch({
    ...
    write("DensUMAP failed", file = paste0(output_dir, "/failed_DensUMAP.txt"))
...
#output_dir = '/home/jarcos/Documents/Real_Data/CalorieRestriction/Aorta'
...
ReorderRefClusters <- function(seur) {
  ...
  seur@meta.data$RefinedClusters <- factor(seur@meta.data$RefinedClusters, levels = as.character(sorted_RefinedClusters), ordered = TRUE)
...
MergeClusters <- function(seur, clusters_to_merge, use_regex = FALSE) {
  ...
  if (use_regex) {
    ...
    # Reorder the clusters after merging
    seur <- ReorderRefinedClusters(seur)
  }
...
umap <- function(seur, genes = NULL) {
  if (is.null(genes)) {
    ...
    plot <- scFeaturePlot(seur, features = genes, colors_use = viridis_plasma_dark_high, na_color = "lightgray")
  }
  return(plot)
}
